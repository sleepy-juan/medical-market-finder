...
export const HyExport = {
  encode(message: HyExport, writer: Writer = Writer.create()): Writer {
    if (message.index !== "") {
  ...
  decode(input: Reader | Uint8Array, length?: number): HyExport {
    const reader = input instanceof Uint8Array ? new Reader(input) : input;
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = { ...baseHyExport } as HyExport;
  ...
  toJSON(message: HyExport): unknown {
    const obj: any = {};
  ...
  fromPartial(object: DeepPartial<HyExport>): HyExport {
    const message = { ...baseHyExport } as HyExport;
    if (object.index !== undefined && object.index !== null) {
